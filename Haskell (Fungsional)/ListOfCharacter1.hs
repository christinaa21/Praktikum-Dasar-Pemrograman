module ListOfCharacter1 where
    ...
    {- makeUnique(lc) menerima masukan sebuah list of character, misalnya lc,
    dan menghasilkan list dengan elemen-elemen unik, yaitu, kemunculan setiap
    elemen (jika tidak kosong), hanya 1-}
    cekElemen :: [Char] -> Char -> Bool
    {- cekElemen(lc, x) mengecek apakah suatu char (x) merupakan anggota
    dari suatu list of character (lc) -}
    -- REALISASI
    cekElemen lc x
        | isEmpty lc = False 
        | head lc == x = True 
        | otherwise = cekElemen (tail lc) x
    makeUnique lc
        | isEmpty lc = []
        | cekElemen (makeUnique (init lc)) (last lc) = makeUnique (init lc)
