module ListOfCharacter2 where
    ...
    dan menghasilkan list, misalnya lc’, yang berisi elemen-elemen lc dengan
    urutan yang dibalik. Jika lc = [e1,e2,...en-1,en] maka lc’ = [en,en-1,...,e2,e1].-}
    inverse lc
        | isEmpty lc = []                                   -- Basis
        | otherwise = konsDot (inverse (tail lc)) (head lc) -- Rekurens
    -- CONTOH APLIKASI
    -- inverse ['s','a','y']
    -- "yas"
    -- inverse []
